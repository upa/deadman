...
use Socket qw( SOCK_DGRAM SOCK_STREAM SOCK_RAW SOL_SOCKET SO_ERROR );
use Socket6 qw( PF_INET6 inet_pton inet_ntop sockaddr_in6 );
...
sub new
{
  ...
  croak('Protocol for ping must be "icmp", "udp", "tcp", "syn", "stream"')
    unless $proto =~ m/^(icmp|udp|tcp|syn|stream)$/;
  $self->{"proto"} = $proto;
  ...
  if ($self->{"proto"} eq "udp")              # Open a socket
  {
    ...
    socket($self->{"fh"}, PF_INET6, SOCK_DGRAM,
           $self->{"proto_num"}) ||
  ...
  elsif ($self->{"proto"} eq "icmp")
  {
    ...
    $self->{"fh"} = FileHandle->new();
    socket($self->{"fh"}, PF_INET6, SOCK_RAW, $self->{"proto_num"}) ||
...
sub bind
{
  ...
  $ip = inet_pton(PF_INET6, $local_addr);
  ...
  {
  CORE::bind($self->{"fh"}, sockaddr_in6(0, $ip)) ||
...
sub ping
{
  ...
  return wantarray ? ($ret, &time() - $ping_time, inet_ntop($ip)) : $ret;
}
...
sub ping_icmp
{
  ...
  $checksum = Net::Ping6->checksum($msg);
  $msg = pack(ICMP_STRUCT . $self->{"data_size"}, ICMP_ECHO, SUBCODE,
              $checksum, $self->{"pid"}, $self->{"seq"}, $self->{"data"});
  $len_msg = length($msg);
  $saddr = sockaddr_in6(ICMP_PORT, $ip);
  $self->{"from_ip"} = undef;
...
sub icmp_result {
  ...
  return (inet_ntop($ip),($self->{"from_type"} || 0), ($self->{"from_subcode"} || 0));
}
...
sub tcp_connect
{
  ...
  $saddr = sockaddr_in6($self->{"port_num"}, $ip);

  my $ret = 0;            # Default to unreachable

  my $do_socket = sub {
    socket($self->{"fh"}, PF_INET6, SOCK_STREAM, $self->{"proto_num"}) ||
      croak("tcp socket error - $!");
    if (defined $self->{"local_addr"} &&
        !CORE::bind($self->{"fh"}, sockaddr_in6(0, $self->{"local_addr"}))) {
...
sub ping_syn
{
  ...
  my $saddr = sockaddr_in6($self->{"port_num"}, $ip);

  # Create TCP socket
  if (!socket ($fh, PF_INET6, SOCK_STREAM, $self->{"proto_num"})) {
    croak("tcp socket error - $!");
  }

  if (defined $self->{"local_addr"} &&
      !CORE::bind($fh, sockaddr_in6(0, $self->{"local_addr"}))) {
    croak("tcp bind error - $!");
  }
...
sub ping_syn_fork {
  ...
  if (defined $pid) {
    if ($pid) {
      ...
      # Create TCP socket
      if (!socket ($self->{"fh"}, PF_INET6, SOCK_STREAM, $self->{"proto_num"})) {
        croak("tcp socket error - $!");
      }
    ...
    while ($wbits !~ /^\0*\z/) {
      ...
      if (defined($winner_fd) or my $nfound = mselect(undef, ($wout=$wbits), undef, $timeout)) {
        ...
        if (my $entry = $self->{"syn"}->{$fd}) {
          ...
          return wantarray ?
            ($entry->[0], &time() - $entry->[3], inet_ntop($entry->[1]))
            : $entry->[0];
        } else {
...
sub ack_unfork {
  ...
  if ($host) {
    ...
    if (my $entry = $self->{"good"}->{$host}) {
      ...
      return ($entry->[0], &time() - $entry->[3], inet_ntop($entry->[1]));
    }
  }
...
Net::Ping6 - check a remote host for reachability
...
=item Net::Ping6->new([$proto [, $def_timeout [, $bytes [, $device [, $tos ]]]]]);
...
application specific and is beyond the scope of Net::Ping6.
...
The latest Net::Ping6 release can be found at CPAN:
...
$Id: Ping6.pm,v 1.1.1.1 2010/11/28 15:36:17 upa Exp $
